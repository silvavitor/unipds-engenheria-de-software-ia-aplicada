...
describe('Chat de Recomendação Musical - Testes E2E', () => {
  ...
  it('Deve manter sumarizar da conversa', async () => {
    const testThreadId = `test-user-${Date.now()}`;
    const config = {
      configurable: { thread_id: testThreadId },
      context: { userId: testThreadId },
    ...
    await graph.invoke(
      {
        messages: [
          new HumanMessage('Sou o alfredo'),
          new HumanMessage('gosto de musicas animadas'),
          new HumanMessage('rock'),
          new HumanMessage('pop'),
          new HumanMessage('tenho 20 anos'),
          new HumanMessage('tenho 5 cachorros'),
          new HumanMessage('não gosto de samba'),
        ],
        userId: testThreadId,
      },
      config
